...
# Basic Agglomerative (default n_clusters=3; linkage='ward' is default but memory-intensive)
agg = AgglomerativeClustering(n_clusters=3, linkage='ward')  # FIX: Use 'ward' (Euclidean); for large data, consider 'average' if issues persist
labels = agg.fit_predict(X_cluster_sample)
...
if unique_labels > 1:
    ...
    print(f"Agglomerative (Basic): Silhouette={sil:.2f}, DB Index={db:.2f}")
else:
    print("Agglomerative (Basic): Only one cluster detected; scores not computable.")
...
joblib.dump({'model': agg, 'sample_size': n_sample, 'sample_idx': sample_idx if 'sample_idx' in locals() else None}, 'data/models/clustering/agglomerative_basic.pkl')

# Plot clusters
plt.figure(figsize=(8, 6))
scatter = plt.scatter(df_sample['loc_pca1'], df_sample['loc_pca2'], c=labels, cmap='viridis')
plt.colorbar(scatter, label='Cluster')
plt.title(f'Agglomerative Clusters (n={n_sample} samples)')
plt.xlabel('loc_pca1')
plt.ylabel('loc_pca2')
plt.show()