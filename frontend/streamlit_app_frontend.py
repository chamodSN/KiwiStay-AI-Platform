...
r2_display = f"{r2_value[0]:.2f}" if len(
    r2_value) > 0 and pd.notnull(r2_value[0]) else 'Not Available'
...
if 'availability_365_calculated_host_listings_count' in X_train.columns:
    input_data['availability_365_calculated_host_listings_count'] = availability_365 * \
        calculated_host_listings_count
if 'minimum_nights_cleaned_calculated_host_listings_count' in X_train.columns:
    input_data['minimum_nights_cleaned_calculated_host_listings_count'] = minimum_nights * \
        calculated_host_listings_count
...
# Scale numerical & polynomial features
numerical_cols = ['latitude', 'longitude', 'minimum_nights_cleaned', 'number_of_reviews',
...
scaler_cols = [col for col in numerical_cols +
               poly_cols if col in X_train.columns]
