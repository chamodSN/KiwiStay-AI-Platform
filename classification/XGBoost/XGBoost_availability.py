...
try:
    ...
    y_avail_train = joblib.load('data/processed/y_avail_train.pkl')
    X_avail_test = joblib.load('data/processed/X_avail_test.pkl')
    y_avail_test = joblib.load('data/processed/y_avail_test.pkl')
    print(
        f"Loaded availability data: Train shape {X_avail_train.shape}, Test shape {X_avail_test.shape}")
...
X_train = X_avail_train.loc[:, ~X_avail_train.columns.duplicated()]
X_test = X_avail_test.loc[:, ~X_avail_test.columns.duplicated()]
...
y_train = y_avail_train.astype(int)
y_test = y_avail_test.astype(int)
...
print(
    f"XGBoost Availability (Optimized): Best Params={grid.best_params_}, Accuracy={acc:.2f}, F1={f1:.2f}, ROC-AUC={roc_auc:.2f}")

# Save model
os.makedirs('data/models/classification', exist_ok=True)
joblib.dump(best_model, 'data/models/classification/xgb_availability.pkl')
...
plt.plot([0, 1], [0, 1], 'k--')
plt.title('XGBoost Availability ROC Curve')
...
plt.title('XGBoost Availability Confusion Matrix')
